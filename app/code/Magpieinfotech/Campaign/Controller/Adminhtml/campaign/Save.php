...
class Save extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($data) {
            $model = $this->_objectManager->create('Magpieinfotech\Campaign\Model\Campaign');
			...
			try{
				...
				$result = $uploader->save($mediaDirectory->getAbsolutePath('campaign_campaign'));
					if($result['error']==0)
					{
						$data['image'] = 'campaign_campaign' . $result['file'];
					}
			} catch (\Exception $e) {
				//unset($data['image']);
            }
